Public Class MonthlyStatement
    Public Property CompanyName As String
    Public Property CusName As String
    Public Property CusCode As String
    Public Property ToGo As MonthlyStatementDetail
    Public Property Delivery As MonthlyStatementDetail = Nothing
    Public Property InsuranceUnitPrice As Single
    Public Property GasAccountsReceived As Integer
    Public Property NewBerralAccountsReceivable As Integer
    Public Property IsInsurance As Boolean
    Public Property ScrapBarrel As Integer
End Class

Public Class MonthlyStatementDetail
    Public Property GasNormalQuantity_First As Integer
    Public Property GasNormalUnitPrice_First As Single
    Public Property GasCQuantity_First As Integer
    Public Property GasCUnitPrice_First As Single
    Public Property GasNormalQuantity As Integer
    Public Property GasNormalUnitPrice As Single
    Public Property GasCQuantity As Integer
    Public Property GasCUnitPrice As Single
End Class
