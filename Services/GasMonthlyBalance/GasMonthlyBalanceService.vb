Public Class GasMonthlyBalanceService
    ...
    Public Async Sub UpdateOrAdd(gmbRep As IGasMonthlyBalanceRep, ordRep As IOrderRep, compRep As ICompanyRep, bsRep As IBasicSetRep, month As Date, companyId As Integer) Implements IGasMonthlyBalanceService.UpdateOrAdd
        Try
            Dim comp = Await compRep.GetByIdAsync(companyId)

            '取得當月所有採購資料
            Dim purDatas = comp.purchases.Where(Function(x) x.pur_date.Value.Year = month.Year AndAlso x.pur_date.Value.Month = month.Month)

            '取得當月所有銷售資料
            Dim saleDatas = ordRep.GetByMonthAndCompany(month, companyId)

            '取得上期結餘
            Dim lastBalance = Await GetLastClosingBalanceAsync(compRep, bsRep, month, companyId)

            '更新或新增月結餘資料
            Dim gmb = gmbRep.GetByMonthAndCompany(month, companyId)
            Dim totalPurchases = purDatas.Sum(Function(x) x.pur_quantity)
            Dim totalSales = saleDatas.Sum(Function(x) x.o_gas_total + x.o_gas_c_total)
            Dim closingBalance = lastBalance + totalPurchases - totalSales
            If gmb Is Nothing Then
            ...
            '更新後續月份
            UpdateSubsequentMonths(compRep, month, companyId, closingBalance)

            ' 注意：不調用 SaveChangesAsync，由 UnitOfWork 統一管理
        Catch ex As Exception
            Throw
        End Try
    End Sub
    ...
    ' 新增輔助方法（給新版本使用）
    Private Async Function GetLastClosingBalanceAsync(compRep As ICompanyRep, bsRep As IBasicSetRep, month As Date, compId As Integer) As Task(Of Integer)
        Try
            Dim comp = Await compRep.GetByIdAsync(compId)
            Dim formatDate = New Date(month.Year, month.Month, 1)
            Dim gmb = comp.gas_monthly_balances.Where(Function(x) x.gmb_Month < formatDate).OrderByDescending(Function(x) x.gmb_Month).FirstOrDefault
            Dim bs = bsRep.GetAllAsync().Result.FirstOrDefault
    ...
    Private Async Sub UpdateSubsequentMonths(compRep As ICompanyRep, month As Date, compId As Integer, currentClosingBalance As Integer)
        Try
            Dim comp = Await compRep.GetByIdAsync(compId)
            Dim afterMonths = comp.gas_monthly_balances.Where(Function(x) x.gmb_Month > month).OrderBy(Function(x) x.gmb_Month)
