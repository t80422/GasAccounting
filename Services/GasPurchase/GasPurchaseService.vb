...
Public Class GasPurchaseService
    ...
    Public Function GetPurchaseTradeSummary(datas As List(Of purchase), criteria As PurchaseCondition) As Tuple(Of List(Of PurchaseGasVendorTradeSummaryListVM), List(Of PurchaseFreightTradeSummaryListVM)) Implements IGasPurchaseService.GetPurchaseTradeSummary
        Try
            ...
            ' 取得大氣廠商分組總結（過濾掉廠商 ID 或 manufacturer 為 null 的記錄）
            Dim gasVendorSummary = datas.Where(Function(x) x.pur_manu_id.HasValue AndAlso x.manufacturer IsNot Nothing) _
                                         .GroupBy(Function(x) x.pur_manu_id.Value) _
            ...
            ' 取得運輸廠商分組總結（過濾掉 manufacturer1 為 null 的記錄）
            Dim transportationSummary = datas.Where(Function(x) x.pur_DriveCmpId.HasValue AndAlso x.manufacturer1 IsNot Nothing) _
                                              .GroupBy(Function(x) x.pur_DriveCmpId.Value) _
                                              .Select(Function(g) New With {
                                                .vendorId = g.Key,
            ...
            ' 取得涉及的運輸廠商ID
            Dim transportationIds = transportationSummary.Select(Function(x) x.vendorId).ToList
    ...
    Public Function GetPurchaseTradeSummary(paymentRep As IPaymentRep, datas As List(Of purchase), criteria As PurchaseCondition) As Tuple(Of List(Of PurchaseGasVendorTradeSummaryListVM), List(Of PurchaseFreightTradeSummaryListVM)) Implements IGasPurchaseService.GetPurchaseTradeSummary
        Try
            Dim paymentCriteria = New PaymentSearchCriteria With {
                .IsSearchDate = criteria.IsDateSearch,
                .StartDate = criteria.StartDate,
            ...
            ' 查詢大氣付款資料（使用傳入的 paymentRep）
            Dim gasVendorPayment = paymentRep.GetByCriteriaAndVendors(paymentCriteria, gasVendorIds)
            Dim gasPaymentSummary = gasVendorPayment.GroupBy(Function(x) x.p_m_Id).
                                                     Select(Function(g) New With {
                                                        .vendorId = g.Key,
                                                        .paid = g.Sum(Function(x) x.p_Amount)
            ...
            ' 查詢運費付款資料（使用傳入的 paymentRep）
            Dim transportationPayment = paymentRep.GetByCriteriaAndVendors(paymentCriteria, transportationIds)
            Dim transportationPaymentSummary = transportationPayment.GroupBy(Function(x) x.p_m_Id).
                                                                     Select(Function(g) New With {
                                                                        .vendorId = g.Key,
                                                                        .paid = g.Sum(Function(x) x.p_Amount)
